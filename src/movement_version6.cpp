...
int main(int argc, char **argv)
{
    ...
    for (float alpha = -180; alpha < 180; alpha += 20)
    {
        ...
        temp.translation() = Eigen::Vector3d(p1.position.x, p1.position.y - r * (180 - abs(alpha)) / 180, p1.position.z);
        ...
        if (alpha == 160)
        {
            for (float beta = -180; beta < 180; beta += 20)
            {
                ...
                temp.translation() = Eigen::Vector3d(p1.position.x, p1.position.y + r * (180 - abs(beta)) / 180, p1.position.z);
